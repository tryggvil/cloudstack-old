...
package com.vmops.capacity.dao;
...
public class CapacityDaoImpl extends GenericDaoBase<CapacityVO, Long> implements CapacityDao {
    ...
    private static final String ADD_ALLOCATED_SQL = "UPDATE `vmops`.`op_host_capacity` SET used_capacity = used_capacity + ? WHERE host_id = ? AND capacity_type = ?";
    private static final String SUBTRACT_ALLOCATED_SQL = "UPDATE `vmops`.`op_host_capacity` SET used_capacity = used_capacity - ? WHERE host_id = ? AND capacity_type = ?";
    private static final String SET_USED_STORAGE_SQL = "UPDATE `vmops`.`op_host_capacity` SET used_capacity = ? WHERE host_id = ? AND capacity_type = 2";
    private static final String CLEAR_STORAGE_CAPACITIES = "DELETE FROM `vmops`.`op_host_capacity` WHERE capacity_type=2 OR capacity_type=6"; //clear storage and secondary_storage capacities
    private static final String CLEAR_NON_STORAGE_CAPACITIES = "DELETE FROM `vmops`.`op_host_capacity` WHERE capacity_type<>2 AND capacity_type <>6"; //clear non-storage and non-secondary_storage capacities
