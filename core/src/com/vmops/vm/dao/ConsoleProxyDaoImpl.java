...
public class ConsoleProxyDaoImpl extends GenericDaoBase<ConsoleProxyVO, Long> implements ConsoleProxyDao {
    ...
    private static final String PROXY_ASSIGNMENT_MATRIX =
    	...
    	" (SELECT v.id AS id, uv.proxy_id AS proxy_id FROM vm_instance AS v JOIN user_vm AS uv ON v.id=uv.id AND "  +
    ...
    private static final String DATACENTER_VM_MATRIX =
    	...
    	" WHERE v.type='User' AND (v.state='Creating' OR v.state='Starting' OR v.state='Running' OR v.state='Migrating')"  +
    ...
    private static final String GET_PROXY_LOAD =
    	...
    	" FROM vm_instance AS v JOIN user_vm AS uv ON v.id = uv.id" 											+
    	" WHERE uv.proxy_id=? AND (v.state='Running' OR v.state='Starting' OR v.state='Creating' OR v.state='Migrating')";
