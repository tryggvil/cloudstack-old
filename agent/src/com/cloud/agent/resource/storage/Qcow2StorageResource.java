...
import com.cloud.storage.FileSystemStorageResource;
...
public class Qcow2StorageResource extends FileSystemStorageResource {
    ...
    protected Answer execute(final PrimaryStorageDownloadCommand cmd) {
    	...
    	try {
    		...
    		return new DownloadAnswer(null, 100, cmd, com.cloud.storage.VMTemplateStorageResourceAssoc.Status.DOWNLOAD_ERROR, null,
        			null);
    	}
    	if (ss.getScheme().equalsIgnoreCase("nfs")) {
    		if (!isNfsMounted(remoteHost, sharePath, _secondaryStroageMountPoint)) {
    			...
    			if (result != null) {
    				...
    				return new DownloadAnswer(null, 0, cmd, com.cloud.storage.VMTemplateStorageResourceAssoc.Status.DOWNLOAD_ERROR, null,
    						null);
    			}
    		}
    	...
    	return new DownloadAnswer(null, 100, cmd, com.cloud.storage.VMTemplateStorageResourceAssoc.Status.DOWNLOADED, installPath + fileName,
    ...
    protected CreatePrivateTemplateAnswer execute(CreatePrivateTemplateCommand cmd) {
        ...
        if (result == null) {
            FormatInfo info = _processor.process(temlatePath, null, cmd.getTemplateName());
            ImageFormat format = info.format;
            answer = new CreatePrivateTemplateAnswer(cmd, (result == null), result, null, info.virtualSize, null, format);
    ...
    protected GetStorageStatsAnswer execute(final GetStorageStatsCommand cmd) {
    	...
    	final long cap = getTotalSize(cmd.getLocalPath());
    	return size != -1 ? new GetStorageStatsAnswer(cmd, cap, size) : new GetStorageStatsAnswer(cmd, "Unable to get storage stats");
    }
