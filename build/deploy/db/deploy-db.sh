...
if [ "$1" == "" ]; then
  printf "Usage: %s [path to server-setup.sql] [path to additional sql]\n" $(basename $0) >&2
  exit 1;
fi
...
mysql --user=root < create-database.sql > /dev/null 2>/dev/null
mysqlout=$?
...
# Run server-setup.sql on the database
mysql --user=vmops --password=vmops vmops < $1
if [ $? -ne 0 ]; then
	printf "Error: Cannot execute $1\n"
	exit 12
fi

# Run the optional SQL file on the database
if [ "$2" != "" ]; then
