...
class ISCSISR(SR.SR):
    ...
    def load(self, sr_uuid):
        ...
        try:
            self.target = util._convertDNS(self.dconf['target'])
        except:
        ...
        if self.dconf.has_key('chappassword'):
            pbd = None
            try:
                pbd = util.find_my_pbd(self.session, self.host_ref, self.sr_ref)
            except:
                pass
            if pbd <> None:
                # UPGRADE FROM RIO/Miami beta1: obfuscate any chappassword
                util.SMlog("Performing chappassword upgrade from Rio/Miami beta1")

                device_config = self.session.xenapi.PBD.get_device_config(pbd)
                device_config['chappassword_transformed'] = util.transform_string(self.dconf['chappassword'])
                del device_config['chappassword']
                self.session.xenapi.PBD.set_device_config(pbd, device_config)            

        # De-obfuscate any chappassword for use:
        if self.dconf.has_key('chappassword_transformed') and not(self.dconf.has_key('chappassword')):
            self.dconf['chappassword'] = util.untransform_string(self.dconf['chappassword_transformed'])

        # Optional parameters
        self.chapuser = ""
        self.chappassword = ""
        if self.dconf.has_key('chapuser') and self.dconf.has_key('chappassword'):
    ...
    def attach(self, sr_uuid):
        ...
        sessions = len(self.adapter.iterkeys())
        try:
            pbdref = util.find_my_pbd(self.session, self.host_ref, self.sr_ref)
            if pbdref <> None:
                self.session.xenapi.PBD.add_to_other_config(pbdref, "iscsi_sessions", str(sessions))
        except:
            pass
    ...
    def _getLUNbySMconfig(self, sm_config):
        ...
        LUNid = long(self.sm_config['LUNid'])
