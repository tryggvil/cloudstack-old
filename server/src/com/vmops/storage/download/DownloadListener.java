...
import com.vmops.storage.download.DownloadState.DownloadEvent;
...
public class DownloadListener implements Listener {
	...
	public void logDisconnect() {
		...
		downloadMonitor.logEvent(EventTypes.EVENT_TEMPLATE_DOWNLOAD_FAILED, "Storage server " + sserver.getName() + " disconnected during download of template " + template.getName(), EventVO.LEVEL_WARN);
	}
	...
	private synchronized void transition(DownloadEvent event, Object evtObj) {
		...
		if (nextState != null) {
			...
			if (currState != null) {
				...
				throw new VmopsRuntimeException("Invalid next state: currState="+prevName+", evt="+event + ", next=" + nextState);
			}
		} else {
			throw new VmopsRuntimeException("Unhandled event transition: currState="+prevName+", evt="+event);
		}
	}
	...
	public void logDownloadStart() {
		downloadMonitor.logEvent(EventTypes.EVENT_TEMPLATE_DOWNLOAD_START, "Storage server " + sserver.getName() + " started download of template " + template.getName(), EventVO.LEVEL_INFO);
	}
