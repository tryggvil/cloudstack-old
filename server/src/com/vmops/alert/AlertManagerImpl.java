            ...
            try {
                ...
                for (HostVO host : hosts) {
                    ...
                    if (host.getType() == Host.Type.Storage) {
                        long disk = 0l;
                        List<VolumeVO> volumes = _volumeDao.findByHost(host.getId());
                        for (VolumeVO volume : volumes) {
                            disk += volume.getSize();
                        }
                        CapacityVO newStorageCapacity = new CapacityVO(host.getId(), host.getDataCenterId(), host.getPodId(), disk, host.getTotalSize()*_overProvisioningFactor, CapacityVO.CAPACITY_TYPE_STORAGE_ALLOCATED);
                    ...
                    CapacityVO newMemoryCapacity = new CapacityVO(host.getId(), host.getDataCenterId(), host.getPodId(), usedMemory, (host.getTotalMemory().longValue()*7L)/8L, CapacityVO.CAPACITY_TYPE_MEMORY);
