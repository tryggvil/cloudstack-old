...
public class RollbackSnapshotExecutor extends VMOperationExecutor {
    public static final Logger s_logger = Logger.getLogger(RollbackSnapshotExecutor.class.getName());
	...
	public boolean execute() {
		...
		if(getSyncSource() == null) {
	    	...
	    	try {
				if(asyncMgr.getExecutorContext().getManagementServer().rollbackToSnapshot(param.getUserId(), param.getSnapshotId()))
					asyncMgr.completeAsyncJob(getJob().getId(), 
			    		AsyncJobResult.STATUS_SUCCEEDED, 0, "success");
				else
					asyncMgr.completeAsyncJob(getJob().getId(), 
			    		AsyncJobResult.STATUS_FAILED, BaseCmd.INTERNAL_ERROR, "failed to rollback to snapshot");
	    	} catch(Exception e) {
	    		s_logger.warn("Unable to rollback snapshot " + param.getSnapshotId() + ":" + e.getMessage(), e);
				asyncMgr.completeAsyncJob(getJob().getId(), 
