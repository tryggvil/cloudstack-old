...
public class ListHostStatsCmd extends BaseCmd {
	public static final Logger s_logger = Logger.getLogger(ListHostStatsCmd.class.getName());
    private static final String s_name = "listhoststatsresponse";
    ...
    static {
    	...
    	s_properties.add(new Pair<Enum, Boolean>(BaseCmd.Properties.HOST_IDS, Boolean.FALSE));
    }
    ...
    public List<Pair<String, Object>> execute(Map<String, Object> params) {
    	...
    	String hostIdsString = (String) params.get(BaseCmd.Properties.HOST_IDS.getName());
    	
    	if ((hostId == null) && (hostIdsString == null)) {
            throw new ServerApiException(BaseCmd.PARAM_ERROR, "No host id (or list of ids) specified.");
        }
    	
    	List<Long> hostIds = new ArrayList<Long>();
    	if (hostIdsString == null) { 
    		hostIds.add(hostId);
    	} else {
    		String[] hostIdsStringArray = hostIdsString.split("//,");
    		for (String hostIdString : hostIdsStringArray) {
    			try {
    				hostIds.add(Long.valueOf(hostIdString));
    			} catch (Exception e) {
    				throw new ServerApiException(BaseCmd.PARAM_ERROR, "Please specify valid host IDs.");
    			}
    		}
    	}
    	...
    	try {
    		long jobId = getManagementServer().listHostStatisticsAsync(hostIds);
    		
    		if (jobId == 0) {
            	s_logger.warn("Unable to schedule async-job for ListHostStats command");
            } else {
    	        if(s_logger.isDebugEnabled())
    	        	s_logger.debug("ListHostStats command has been accepted, job id: " + jobId);
            }
    		...
    		throw new ServerApiException(BaseCmd.INTERNAL_ERROR, "Failed to list host stats: " + ex.getMessage());
    	}
