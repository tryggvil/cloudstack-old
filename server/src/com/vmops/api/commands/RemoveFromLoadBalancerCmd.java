...
public class RemoveFromLoadBalancerCmd extends BaseCmd {
    public static final Logger s_logger = Logger.getLogger(RemoveFromLoadBalancerCmd.class.getName());

    private static final String s_name = "removefromloadbalancerresponse";
    ...
    public List<Pair<String, Object>> execute(Map<String, Object> params) {
        ...
        if (loadBalancer == null) {
            ...
            if (!isAdmin(account.getType()) && (loadBalancer.getAccountId() != account.getId().longValue())) {
                ...
                throw new ServerApiException(BaseCmd.PARAM_ERROR, "Invalid load balancer id (" + loadBalancer.getId() + ") given, unable to remove virtual machine instances.");
            }
        }

        long jobId = getManagementServer().removeFromLoadBalancerAsync(userId.longValue(), loadBalancerId.longValue(), instanceIdList);
        if(jobId == 0) {
        	s_logger.warn("Unable to schedule async-job for RemoveFromLoadBalancer comamnd");
        } else {
	        if(s_logger.isDebugEnabled())
	        	s_logger.debug("RemoveFromLoadBalancer command has been accepted, job id: " + jobId);
        }
