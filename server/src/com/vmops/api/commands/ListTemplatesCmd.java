...
public class ListTemplatesCmd extends BaseCmd {
    ...
    static {
        ...
        s_properties.add(new Pair<Enum, Boolean>(BaseCmd.Properties.SHOW_ALL, Boolean.FALSE));
    }
    ...
    public List<Pair<String, Object>> execute(Map<String, Object> params) {
        ...
        Boolean showAll = (Boolean)params.get(BaseCmd.Properties.SHOW_ALL.getName());
        ...
        if ((!showAll.booleanValue()) && ((domRTemplate != null) && (domRTemplate.isReady() == false))) {
            templates = new ArrayList<VMTemplateVO>();
        } else {
            if (id != null) {
                templates.add(getManagementServer().findTemplateById(id.longValue()));
            } else {
                templates = getManagementServer().listTemplates(name, keyword, isReady, isPublic, false, null, accountId, Integer.valueOf(pageSizeNum), startIndex);
            }

            // if showAll is false, for admins remove the domR template from the list of VMs
            if (isAdmin && !showAll.booleanValue()) {
        ...
        Object[] tTag = new Object[templates.size()];
        int i = 0;
