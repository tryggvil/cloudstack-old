...
public class PodZoneConfig {
	...
	public void run(String[] args) {
		...
		if (podOrZone.equals("pod")) {
			...
			savePod(true, -1, podName, dcId, cidr);
	...
	public List<String> savePodGUI(String podName, String zoneName, String cidr) {
		...
		savePod(false, -1, podName, dcId, cidr);
		
		return DatabaseConfig.genReturnList("true", "");
	}
	
	public List<String> saveZoneGUI(String zoneName, String gateway, String netmask, String dns1, String dns2, String dns3, String dns4) {
		...
		saveZone(false, -1, zoneName, dns1, dns2, dns3, dns4, -1, -1);
		
		return DatabaseConfig.genReturnList("true", "");
	}
	
	public void savePod(boolean printOutput, long id, String name, long dcId, String cidr) {
		...
		String sql = null;
		if (id != -1) sql = "INSERT INTO `vmops`.`host_pod_ref` (id, name, data_center_id, cidr_address, cidr_size) " + "VALUES ('" + id + "','" + name + "','" + dcId + "','" + cidrAddress + "','" + cidrSize + "')";
		else sql = "INSERT INTO `vmops`.`host_pod_ref` (name, data_center_id, cidr_address, cidr_size) " + "VALUES ('" + name + "','" + dcId + "','" + cidrAddress + "','" + cidrSize + "')";
			
        DatabaseConfig.saveSQL(sql, "Failed to save pod due to exception. Please contact VMOps Support.");
	...
	private String checkPodCidrSubnets(long dcId, HashMap<Long, Vector<Object>> currentPodCidrSubnets) {
		...
		String guestIpNetwork = IPRangeConfig.getGuestIpNetwork();
		long guestCidrSize = IPRangeConfig.getGuestCidrSize();
	...
	protected HashMap<Long, Vector<Object>> getCurrentPodCidrSubnets(long dcId) {
		...
		try {
        	...
        	printError("There was an issue with reading currently saved pod CIDR subnets. Please contact VMOps Support.");
            return null;
        }
	...
	public void deletePod(String name, long dcId) {
		String sql = "DELETE FROM `vmops`.`host_pod_ref` WHERE name=\"" + name + "\" AND data_center_id=\"" + dcId + "\"";
		DatabaseConfig.saveSQL(sql, "Failed to delete pod due to exception. Please contact VMOps Support.");
	}
	
	public long getVlanDbId(String zone, String vlanId) {
		long zoneId = getZoneId(zone);
		
		return DatabaseConfig.getDatabaseValueLong("SELECT * FROM `vmops`.`vlan` WHERE data_center_id=\"" + zoneId + "\" AND vlan_id =\"" + vlanId + "\"", "id",
		"Unable to start DB connection to read vlan DB id. Please contact VMOps Support.");
    }
    	...
    	if (add) {
    		...
    		*/
            ...
            if (podId != null) {
            	...
            	String sql = "INSERT INTO `vmops`.`pod_vlan_map` (pod_id, vlan_db_id) " + "VALUES ('" + podId + "','" + vlanDbId  + "')";
                DatabaseConfig.saveSQL(sql, "Failed to save pod_vlan_map due to exception vlanDbId=" + vlanDbId + ", podId=" + podId + ". Please contact VMOps Support.");
            }
	...
	@DB
	public void saveZone(boolean printOutput, long id, String name, String dns1, String dns2, String dns3, String dns4, int vnetStart, int vnetEnd) {
		...
		String sql = "INSERT INTO `vmops`.`data_center` " + columns +  " VALUES " + values;
		DatabaseConfig.saveSQL(sql, "Failed to save zone due to exception. Please contact VMOps Support.");
        ...
        try {
            ...
            printError("Error creating vnet for the data center. Please contact VMOps Support.");
        }
	...
	public void deleteZone(String name) {
		String sql = "DELETE FROM `vmops`.`data_center` WHERE name=\"" + name + "\"";
		DatabaseConfig.saveSQL(sql, "Failed to delete zone due to exception. Please contact VMOps Support.");
	}
	
	public void saveVlan(long zoneId, Long podId, String vlanId, String vlanGateway, String vlanNetmask, String vlanType) {
		String sql = "INSERT INTO `vmops`.`vlan` (vlan_id, vlan_gateway, vlan_netmask, data_center_id, vlan_type) " + "VALUES ('" + vlanId + "','" + vlanGateway + "','" + vlanNetmask + "','" + zoneId + "','" + vlanType + "')";
        DatabaseConfig.saveSQL(sql, "Failed to save vlan due to exception. Please contact VMOps Support.");
	}
	...
	public static long getPodId(String pod, String zone) {
		...
		String errorMsg = "Could not read pod ID fro mdatabase. Please contact VMOps Support.";
		return DatabaseConfig.getDatabaseValueLong(selectSql, "id", errorMsg);
	}
	
	public static long getZoneId(String zone) {
		String selectSql = "SELECT * FROM `vmops`.`data_center` WHERE name = \"" + zone + "\"";
		String errorMsg = "Could not read zone ID from database. Please contact VMOps Support.";
		return DatabaseConfig.getDatabaseValueLong(selectSql, "id", errorMsg);
	}
	...
	public Vector<Long> getAllZoneIDs() {
		...
		try {
        	...
        	System.out.println(ex.getMessage());
        	printError("There was an issue with reading zone IDs. Please contact VMOps Support.");
            return null;
        }
	...
	public static String getPodName(long podId, long dcId) {
		return DatabaseConfig.getDatabaseValueString("SELECT * FROM `vmops`.`host_pod_ref` WHERE id=" + podId + " AND data_center_id=" + dcId, "name",
		"Unable to start DB connection to read pod name. Please contact VMOps Support.");
	}
	
	public static String getZoneName(long dcId) {
		return DatabaseConfig.getDatabaseValueString("SELECT * FROM `vmops`.`data_center` WHERE id=" + dcId, "name",
		"Unable to start DB connection to read zone name. Please contact VMOps Support.");
	}
