...
public class UtilsForTest {
	...
	public static final Logger s_logger = Logger.getLogger(UtilsForTest.class.getName());
		...
		if (pattern != null) {
			for (String value : pattern.keySet()) {
				if (!pattern.get(value).equals(params.get(value))) {
					result=false;
					s_logger.info("Tag " + value + " has " + params.get(value) + " while expected value is: " + pattern.get(value));
				}
			}
		}
		return result;
