...
public class ConfigTest extends TestCase{
	...
	public boolean executeTest(){
		...
		for (int i=0; i<commandLst.getLength(); i++) {
		    ...
		    //!!!check if we need to restart a management server!!!
...
//		    NodeList commandNm = commandElmnt.getChildNodes();
		    ...
		    if (api.getName().equals("rebootManagementServer")) {
				...
				if ((api.getError() == true) && (api.getResponseCode() == 200) && (api.getTestCaseId() != null)) {
					...
					error++;
						...
						s_logger.info("Test case " + api.getTestCaseId() + " passed. Command that was supposed to fail, failed - test passed. Command was sent with url " + api.getUrl());
				}
		    }
		}
			if (error != 0)
