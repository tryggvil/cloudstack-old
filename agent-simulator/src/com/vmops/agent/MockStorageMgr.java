...
public class MockStorageMgr implements StorageMgr {
    ...
    public long getTotalSize() {
		return AgentSimulator.getInstance().getHostStorageInMB()*1024*1024L;
	}
	...
	public long getUsedSize() {
		...
		synchronized(this) {
			for(MockVolume volume : storage.values())
				size += volume.getSize()*1024*1024L;
		}
	...
	@Override
    public String create(String templatePath, String imagePath, String userPath, int disksize) {
		if(s_logger.isInfoEnabled())
		...
		synchronized(this) {
			...
			volumeName = getVolumeName(imagePath, 1L);
