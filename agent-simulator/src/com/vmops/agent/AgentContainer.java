...
import com.vmops.agent.AgentSimulator.AgentType;
...
public class AgentContainer extends Agent {
	...
	public AgentContainer(IAgentShell shell, int instanceId, AgentType agentType) {
		super(shell);
		...
		if(agentType == AgentType.Computing) {
			_resource = new AgentComputingResource(this);
		} else if(agentType == AgentType.Routing) {
			_resource = new AgentRoutingResource(this);
		} else {
			_resource = new AgentStorageResource(this);
		}
		
        Map<String, Object> params = PropertiesUtil.toMap(_shell.getProperties());
        try {
			_resource.configure("Agent" + instanceId, params);
